  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center space-x-2 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl border border-[#DCD6CB] bg-[#F4F1EA] hover:bg-white text-[#1A1A1A] transition-all shadow-xs cursor-pointer group"
      ...
      >
        ...
        <div className="text-left hidden xs:block">
          <div className="flex items-center space-x-1.5 leading-tight">
            <span className="text-xs sm:text-sm font-bold font-editorial text-[#1A1A1A] truncate max-w-[120px] sm:max-w-[160px]">
              {isAllSelected ? 'Toutes les boulangeries' : activeBakery?.name || 'Boulangerie'}
            </span>
            <ChevronDown className={`w-3.5 h-3.5 text-[#7A756D] transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
          </div>
          <span className="text-[10px] text-[#7A756D] block leading-none font-editorial truncate max-w-[120px] sm:max-w-[160px]">
            {isAllSelected 
              ? `${bakeries.length} boulangeries actives` 
              : activeBakery?.bakerName ? `Boulanger : ${activeBakery.bakerName}` : 'Périmètre actif'}
          </span>
        </div>
